...
function fillElementWithDraft({ category, title, content, keyword, pic, preImg, hdnImg }, draft) {
    ...
    if (draft.pic) {
        const blob = new Blob([new Uint8Array(atob(draft.pic).length)], { type: 'image/png' });
